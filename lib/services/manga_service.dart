...
class MangaService {
  ...
  Future<List<Manga>> fetchLatestManga({int page = 1, int limit = 32}) async {
    try {
      final response = await _retryRequest(() => _client
          .get(
            Uri.parse(
        ...
        for (final chapterData in data['data']) {
          try {
            if (chapterData['type'] == 'chapter') {
              ...
              final attributes =
                  chapterData['attributes'] as Map<String, dynamic>;
              final relationships =
                  chapterData['relationships'] as List<dynamic>;
              ...
              final chapter = Chapter(
                ...
                sourceUrl: 'https://mangadex.org/chapter/${chapterData['id']}',
                source: 'mangadex',
                index: (double.tryParse(
                            (attributes['chapter'] as String?) ?? '') !=
                        null)
                    ? (double.tryParse(
                                (attributes['chapter'] as String?) ?? '0')! *
                            1000)
                        .round()
                    : 0,
                ...
                translatedLanguage:
                    attributes['translatedLanguage'] as String? ?? 'en',
  ...
  String _constructChapterTitle(
      String? volume, String? chapter, String? title) {
    final parts = <String>[];
